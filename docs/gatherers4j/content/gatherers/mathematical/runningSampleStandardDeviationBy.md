...
This implementation is suitable mapping an arbitrary `Stream<T>` to `BigDecimal` via a `mappingFunction`; for a version that operates directly on a `Stream<BigDecimal>`, see [`runningSampleStandardDeviation()`](/gatherers4j/gatherers/mathematical/runningsamplestandarddeviation/).
...
`runningSampleStandardDeviationBy(Function<T, BigDecimal> mappingFunction)`

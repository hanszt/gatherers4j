...
description: Calculate the simple moving average of `BigDecimal` objects mapped from a `Stream<T>` via a `mappingFunction` and looking back `windowSize` number of elements.

---

### Implementation Notes
This implementation is suitable mapping an arbitrary `Stream<T>` to `BigDecimal` via a `mappingFunction`; for a version that operates directly on a `Stream<BigDecimal>`, see [`simpleMovingAverage()`](/gatherers4j/gatherers/mathematical/simplemovingaverage/).
...
`simpleMovingAverageBy(int windowSize, Function<T, BigDecimal> mappingFunction)`

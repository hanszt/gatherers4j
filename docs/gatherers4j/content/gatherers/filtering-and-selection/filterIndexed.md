...
`filterIndexed(BiPredicate<Integer, T> predicate)`

* `predicate` - A non-null `BiPredicate<Integer,T>` where the `Integer` is the zero-based index of the element being filtered, and the `T` is the element itself.
